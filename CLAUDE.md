...
| ファイル | 役割 |
|---|---|
| `collect.py` | 記事(GoogleニュースRSS)・為替・銅建値(JX金属)・鉄スクラップ(東京製鐵)・金銀(田中貴金属)をあつめて `data.json` を書き出す |
| `manual.json` | アルミ二次地金・LME銅を手で書くところ（無料の自動取得先がないため） |
...
- アルミ二次地金・LME銅の無料の自動取得先がない。当面は `manual.json` の手入力。
  アルミ二次合金の相場は四半期ごとの交渉で決まるので、手入力でも実害は小さい。
